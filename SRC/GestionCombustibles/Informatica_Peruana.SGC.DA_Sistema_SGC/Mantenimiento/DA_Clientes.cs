...
{
    public partial class DA_Clientes:DA_Base
    #region "Listado"
    {
        public static List<BE_clientes> Listar_Clientes(BE_clientes pBE_Clientes)
        {
            IDataReader DataReader = null;
            List<BE_clientes> oBE_Clientes = new List<BE_clientes>();
            try
            {
                SqlCommand cmd = (SqlCommand)db.GetStoredProcCommand("USP_SEL_CLIENTES");
                db.AddInParameter(cmd, "@texto_buscar", DbType.String, pBE_Clientes.Str_texto_buscar);
                cmd.CommandTimeout = Convert.ToInt32(DuracionConexion.corta);
                using (DataReader = db.ExecuteReader(cmd))
                {
                    while (DataReader.Read())
                    {
                        oBE_Clientes.Add(new BE_clientes(DataReader, 1));
                    }
                }
                return oBE_Clientes;
            }
